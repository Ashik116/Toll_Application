...
class _ReportSearchState extends State<ReportSearch> {
                          ...
                          itemBuilder: (context, index) {
                            return SearchReportView(
                                ...
                                child: InkWell(
                                  ...
                                  child: Image(
                                    image: NetworkImage("http://103.150.65.122/image/${data.searchModel.data[index].imagase}",
                                    ),
                                    fit: BoxFit.cover,
                                  ),
                                ),
  ...
  rateCalculation() {
    Select == 'Agro Use' ? rate = 135 :
    Select == 'Rickshaw Van' ? rate = 5 :
    Select == 'Sedan Car' ? rate = 40 :
    Select == '4 Wheeler' ? rate = 60 :
    Select == 'Micro Bus' ? rate = 80 :
    Select == 'Motor Cycle' ? rate = 10 :
    Select == 'Trailer Long' ? rate = 565 :
    Select == 'Heavy Truck' ? rate = 260 :
    Select == 'Medium Truck' ? rate = 200:
    Select == 'Big Bus' ? rate = 150 :
    Select == 'Mini Truck' ? rate = 170 :
    Select == '3/4 Wheeler' ? rate = 20 : rate = 80;
  }
}